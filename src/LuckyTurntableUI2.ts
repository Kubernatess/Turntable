class LuckyTurntableUI2 extends eui.Component {
    ...
    private rotatePointer(): boolean {
        ...
        let rotate: number = (360 / 8) * (this.rand+1);
        let endAngle: number = rotate>180 ? rotate-360 : rotate;
        let startAngle: number = endAngle - 2 * this.degreee;
        if (!this.timer.running && this.degreee<=2 && this.pointer.rotation>=startAngle && this.pointer.rotation<=endAngle) {
